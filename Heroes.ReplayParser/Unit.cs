        ...
        public static void ParseUnitData(Replay replay)
        {
            ...
            if (replay.ReplayBuild >= 38236)
            {
                ...
                foreach (var player in replay.Players.Where(i => heroDeathUnits.ContainsKey(i)))
                    player.Deaths = heroDeathUnits[player].Select(i => new Unit { Group = i.Group,
                        Name = i.Name,
                        PlayerControlledBy = i.PlayerControlledBy,
                        PlayerKilledBy = i.PlayerKilledBy,
                        Team = i.Team,
                        TimeSpanAcquired = i.TimeSpanAcquired,
                        TimeSpanBorn = i.TimeSpanBorn,
                        TimeSpanDied = i.TimeSpanDied,
                        PointBorn = i.PointBorn,
                        PointDied = i.PointDied,
                        UnitKilledBy = i.UnitKilledBy,
                        UnitID = i.UnitID                        
                    }).ToArray();
            }
            ...
            foreach (var player in replay.Players.Where(i => i.HeroUnits.Length == 1 && i.Deaths.Length > 0))
            {
                ...
                //}
                //else
                    fullTimerDeaths.AddRange(player.Deaths.Select(i => new Tuple<TimeSpan,TimeSpan?>(i.TimeSpanBorn,i.TimeSpanDied)));
